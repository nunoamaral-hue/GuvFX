...
beforeEach(() => { jm.fetchJourney.mockReset(); jm.requestWorkspace.mockReset(); jm.confirmAccount.mockReset(); });
afterEach(() => cleanup());
...
describe("HostedWorkspaceJourney", () => {
  ...
  it("requests a workspace with only the broker login (never a password)", async () => {
    ...
    // There is no password field anywhere in the hosted journey.
    expect(container.querySelector('input[type="password"]')).toBeNull();
    fireEvent.change(input, { target: { value: "700900" } });
    fireEvent.click(screen.getByRole("button", { name: /request workspace/i }));
    await waitFor(() => expect(jm.requestWorkspace).toHaveBeenCalledTimes(1));
    const arg = jm.requestWorkspace.mock.calls[0][0];
