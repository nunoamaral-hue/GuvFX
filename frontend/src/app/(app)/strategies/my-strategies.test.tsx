...
type StrategyRow = { id: number; name: string; is_active: boolean };

const { state, apiFetch, fetchJourney } = vi.hoisted(() => {
  const state = {
    status: { armed: true, enabled: false, strategy_id: 10 } as Status,
    // The backing Wayond Strategy row (id 10, is_active true) plus a genuinely-generic strategy.
    strategies: [
      { id: 10, name: "Wayond WIM Strategy", is_active: true },
      { id: 3, name: "London Session Box", is_active: false },
    ] as StrategyRow[],
...
describe("My Strategies — signal-copy dedup", () => {
  ...
  it("status comes from lifecycle, not the backing Strategy.is_active: no 'Active' badge for Wayond", async () => {
    // Only the (active) backing Wayond strategy exists; a naive render would show a green "Active" badge.
    state.strategies = [{ id: 10, name: "Wayond WIM Strategy", is_active: true }];
    render(<MyStrategies />);
    await waitFor(() => expect(screen.getByText("Wayond WIM")).toBeTruthy());
  ...
  it("ambiguous ownership (no single strategy_id): nothing is deduped, product surfaces for attention", async () => {
    // Ambiguous status carries no strategy_id, so the generic list is NOT silently hidden.
