...
| **B5 privilege** | the least-privilege virtual service account | the host, other tenants, Customer Zero | the daemon runs as `NT SERVICE\GuvFXHostedExecutor` (WinSW + installer `sc config obj=` + `SeServiceLogonRight`); primitives write per-account NTFS ACLs / AppLocker tenant fragments / a per-account RemoteApp alias; CZ is never provisioned. |
