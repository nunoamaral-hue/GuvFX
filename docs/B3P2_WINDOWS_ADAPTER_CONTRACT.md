...
| Task presence by exact-name **`GetTask`** + HRESULT classification (TSV, 2026-07-25; superseded the earlier enumeration) | Host-measured under `NT SERVICE\GuvFXBetaAgent`: `GetTask` on a missing task returns `ERROR_FILE_NOT_FOUND` (0x80070002) → absent; on an inaccessible task `ERROR_ACCESS_DENIED` (0x80070005) → UNAVAILABLE, never absent. The former `GetTasks(0)` enumeration required folder-LIST access the least-privilege service lacks (the root folder grants Authenticated Users only write), so it read every present task as absent. Exact-name lookup needs only read on the one task. |
