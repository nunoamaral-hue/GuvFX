...
- **R4 hardening (synthetic):** one shared UTC-instant primitive parses/compares
  canonical `Z` timestamps, preserving every admitted fractional digit (trailing
  zeros are equal; ordering survives past the sixth digit) — used by the timezone
  gate, research point-in-time ordering and manifest timestamps. **R4-R1** makes
  that primitive arbitrary-length-safe (the fraction is a normalized decimal-digit
  ...
  as success); publication validates the request through governed exceptions before
  indexing any field.
