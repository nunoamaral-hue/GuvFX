...
def worker_max_lot(payload) -> float:
    """The per-order lot ceiling: the source-scoped ``max_lot`` from the promotion payload,
    fail-closed to the conservative 0.02 default and bounded by ``WORKER_HARD_MAX_LOT``."""
    try:
        cap = float((payload or {}).get("max_lot") or 0.02)
    except (TypeError, ValueError):
        cap = 0.02
    return min(max(cap, 0.0), WORKER_HARD_MAX_LOT)
